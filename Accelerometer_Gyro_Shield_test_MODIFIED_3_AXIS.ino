...
#include <SPI.h>
// The tactor controller needs these two libraries to function:
#include <Wire.h>
#include <Adafruit_DRV2605.h>"

Adafruit_DRV2605 drv;

//MPU 6000 register addresses (in the computer's memory)
...
//Define a variable to determine the maximum expected acceleration value. This will be used to scale the outputs throughout the program.
 ...
 double xAccelerationGain=1;
 int xAccelerationOffset=0;

 double yAccelerationGain=1;
 int yAccelerationOffset=0;

 double zAccelerationGain=1;
...
// Calibrating the maximum values based on the user-input for expected maximum and minimum (acceleration should not exceed 1.)
  xmax = xmax - min;
  ymax = ymax - min;
  zmax = zmax - min;
...
//*****************************************************
{
        ...
        //in this example the gain (m) and offset (x) are setup to m=1/max and x=0, so raw data is normalized to maximum values of 1 for easier tuning across multiple scenarios.
        //This can be calibrated in the sections above. 
        ...
        delay(100);
          ...
          if (xAccelerationValue >= 0.02) {
            drv.setWaveform(0,1);
            drv.go(); 
            delay(50);
          }
          else if (xAccelerationValue <= -0.02) {
            drv.setWaveform(0,47);
            drv.go();
            delay(50);
          }
          else if (yAccelerationValue >= 0.02) {
          ...
          else if (zAccelerationValue >= 0.02) {
            drv.setWaveform(0,58);
            drv.go();
            delay(50);
          }
          else if (zAccelerationValue <= -0.02) {
            drv.setWaveform(0,31);
            drv.go();
            delay(50);
          }
          else {
